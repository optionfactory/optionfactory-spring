    ...
    public BeanPropertyWriter serialization(Quirks.LocalDateTimeAsIsoInstant ann, BeanPropertyWriter bpw) {
        ...
        if (raw != LocalDateTime.class) {
            throw new IllegalStateException(String.format(
                "Invalid @LocalDateTimeAsIsoInstant placement on property '%s'. Can only be applied to LocalDateTime fields, but found type: %s",
    ...
    public static class Deserializer extends ValueDeserializer<LocalDateTime> {
        ...
        public LocalDateTime deserialize(JsonParser jp, DeserializationContext dc) {
            if (!jp.hasToken(JsonToken.VALUE_STRING)) {
                return dc.reportInputMismatch(targetType, 
                    "Expected a string token representing an ISO instant, but got: %s", jp.currentToken());
            }
            ...
            try {
                ...
                return dc.reportInputMismatch(targetType, 
                    "Text '%s' could not be parsed into a valid ISO Instant.", text);
            }
        }
